...
pub async fn download(query: Option<&str>, all: bool, no_cue: bool, no_folder: bool, filename: Option<String>) -> Result<()> {
    let auth_path = get_config_dir().join("auth.json");
    let token_data = std::fs::read_to_string(&auth_path)
        .map_err(|_| CLIError::AuthExpired)?;
        
    let mut auth: audible_api::auth::AuthInfo = serde_json::from_str(&token_data).map_err(|e| anyhow::anyhow!(e))?;
    if auth.is_expired() {
        info!("Access token expired. Refreshing...");
        auth.refresh_access_token().await?;
        std::fs::write(&auth_path, serde_json::to_string_pretty(&auth).map_err(|e| anyhow::anyhow!(e))?)?;
    }
    
    let client = audible_api::Client::new(auth);
    let library_file = get_config_dir().join("library.json");
    let mut state = crate::state::LibraryState::load(&library_file).map_err(|e| anyhow::anyhow!(e))?;
    ...
    if let Some(q) = query {
        ...
        if matches.is_empty() {
            ...
            return Err(CLIError::Anyhow(anyhow::anyhow!("Multiple books matched the query. Aborting.")));
        }
    } else if all {
        ...
        return Err(CLIError::Anyhow(anyhow::anyhow!("Please specify a query (title or ID) or use --all.")));
    }
    ...
    for mut book in books_to_download {
        ...
        downloader.download(&url, &aax_path).await.map_err(|e| anyhow::anyhow!(e))?;
        
        println!("Download complete! Decrypting to {}...", m4b_file_name);
        use crate::media::Decryptor;
        decryptor.decrypt(&aax_path, &m4b_path, &activation_bytes).map_err(|e| anyhow::anyhow!(e))?;
